...
# Motion-only guidance defaults (text branch is always null).
...
echo "[trace_exec] run_predict_i2v_track_motion_only.sh"
...
if [[ -z "${TRACK_FILE_PATH}" && -z "${METADATA_PATH}" ]]; then
  echo "[error] motion_only requires motion condition input."
